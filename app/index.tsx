...
class TWTable extends React.Component<IN_config, any> {
  constructor(props: IN_config) {
    ...
    this.state = {
      ...
      defaultstyle: Object.prototype.hasOwnProperty.call(
        this.props,
        "defaultstyle"
      )
      ...
      isError: false,
      errorMessage: "",
      downloadableConfig: Object.assign(
        defReportConfig,
        this.props.downloadableConfig
      ),
      styleClass: Object.assign(defClass, this.props.styleClass),
      showbtn: true,
      progress: 0,
  ...
  validateProprty: any = () => {
    ...
    if (serversidePagination && typeof data != "function") {
      isError = true;
      errorMessage =
        "With server side pagination 'data' attribute should be of type Function.";
    } else if (!serversidePagination && !Array.isArray(data)) {
      isError = true;
      errorMessage =
        "With client side pagination 'data' attribute should be of type array of object.";
    }
  ...
  async loadServerSideData(): Promise<void> {
    ...
    const currpage = this.state.recordCount / this.state.pageSize;

    if (currpage < this.state.currentpage) {
      this.changePage(
        currpage * this.state.pageSize,
  ...
  formattheaders: any = async (
    column: string | null = null,
    order: string | null = null
  ) => {
    const { headers } = this.state;

    const tempheaders = headers.map((header: any) =>
      Object.prototype.hasOwnProperty.call(header, "column") &&
  ...
  changePage: any = async (startPage: number, currentpage: number) => {
  ...
  createPagelist: any = (pages: number) => {
    const buttonlist = [];
      ...
      if (currentpage === index) {
        buttonlist.push(
          <button
            ...
            key={index}
            onClick={() => this.changePage(index * this.state.pageSize, index)}
          >
            {index + 1}
          </button>
        );
    ...
    if (currentpage < pages - 1) {
      ...
      buttonlist.push(
        <button
          ...
          key="exnext"
          onClick={() =>
            this.changePage((pages - 1) * this.state.pageSize, pages - 1)
          }
        >
          &#8811;
        </button>
  ...
  rearrangerow: any = async (event: any, column: string, order: string) => {
    ...
    const arrangement = { column, order };
    let recordCount = 0;
    ...
    if (serversidePagination) {
      if (order === "asc") {
        headers = headers.map((header: any) =>
          typeof header.column === "string" && header.column === column
            ? { ...header, order: "desc" }
            : header
        );
      } else {
  ...
  filterServerSideData: any = async (filter: string, event: any) => {
    const eventValue = event.target.value;
    const userfilters = this.state.userfilters;

    userfilters[filter] =
      typeof eventValue === "string" ? eventValue.toLowerCase() : eventValue;
  ...
  filterClientSideData: any = (filter: string, event: any) => {
    const eventValue = event.target.value;
    const userfilters = this.state.userfilters;
  ...
  refreshGrid: any = async () => {
    ...
    this.state.data.forEach((node: any) => {
      ...
      Object.keys(this.state.userfilters).forEach((key: any) => {
        nodeVal =
          typeof node[key] == "string" ? node[key].toLowerCase() : node[key];

        if (
          this.state.userfilters[key].length === 0 ||
          nodeVal.toString().includes(this.state.userfilters[key])
        ) {
  ...
  moveProgressBar: any = (progress: number, message: string) => {
      ...
      <React.Fragment>
        ...
        {isError ? (
          ...
          <Table
            ...
            createPagelist={this.createPagelist}
            headers={headers}
            data={data}
            ...
            rearrangerow={this.rearrangerow}
            pages={pages}
            filter={filter}
            serversidePagination={serversidePagination}
            filterServerSideData={this.filterServerSideData}
            filterClientSideData={this.filterClientSideData}
            sleep={this.sleep}
            startRow={startRow}
            pageSize={pageSize}
