...
const Downloadable: React.FC<any> = ({
  ...
  return (
    <div
      className="col-sm-3 download-file btn-group"
      role="group"
      aria-label="report"
    >
      {showbtn ? (
          ...
          downloadableConfig.reportfn.toLowerCase() === "twtable" ? (
            twtableDownloadSupport.indexOf(option.toUpperCase()) >= 0 ? (
              <button
                type="button"
                key={index}
                className="btn btn-outline-secondary downloadBtn"
                onClick={(event) =>
                  serversidePagination
                    ? getServerReport(event, option)
                    : option.toUpperCase() === "EXCEL"
                    ? Excel(event, headers, filteredData)
                    : CSV(event, headers, filteredData)
                }
              >
                {option}
              </button>
              ...
              console.warn(
                `Download Option ${option} is not intrenally handled by TWtable. Supported options are ${JSON.stringify(
                  twtableDownloadSupport
                )}. Use UDF function for using ${option} download option.`
              )
            )
          ) : (
            <button
